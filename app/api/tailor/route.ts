import {
  validateTailorAnalysis,
  validateTailorInput,
} from "../../../lib/resume-contract.js";
  ...
  properties: {
    ...
    matches: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["title", "source", "evidence", "jdEvidence", "explanation"],
        properties: {
          title: { type: "string" },
          source: { type: "string" },
    ...
    omit: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["title", "original", "reason"],
        properties: {
          title: { type: "string" },
          original: { type: "string" },
          reason: { type: "string" },
...
export async function POST(request: Request) {
  try {
    ...
    const { connection, ...rawInput } =
      body && typeof body === "object" ? body : {};
    const input = validateTailorInput(rawInput);
    const confirmedFacts = input.userConfirmed as {
      question: string;
      answer: string;
    }[];
    ...
    return Response.json(
      { analysis: validateTailorAnalysis(input, JSON.parse(completion.text)) },
